...
function _getConfig() {
  _getConfig = _asyncToGenerator(function* (opts) {
    ...
    webpackConfig.module.rule('images').test(/\.(png|jpe?g|gif|webp)(\?.*)?$/).use('url-loader').loader(require.resolve('url-loader')).options({
    ...
    webpackConfig.plugin('define').use(bundleImplementor.DefinePlugin, [_objectSpread({
      'process.env': (0, _utils2.objToStringified)(_objectSpread({}, process.env, {
        NODE_ENV: env
      }))
    }, (0, _utils2.objToStringified)(config.define || {}))]); // progress
    ...
    if (process.env.SPEED_MEASURE && type === _bundlerUtils().ConfigType.csr) {
